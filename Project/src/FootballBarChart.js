...
const FootballBarChart = ({ data, xAxisColumn, yAxisColumn, samplingMethod }) => {
  ...
  const createBarChart = () => {
    ...
    const xScale = d3
      .scaleBand()
      .domain(sampledData.map((d) => d[xAxisColumn]))
      .range([0, width])
      .padding(0.1);
    ...
    .enter()
    .append('rect')
    .attr('x', (d) => xScale(d[xAxisColumn]))
    .attr('y', (d) => yScale(d[yAxisColumn]))
    .attr('width', xScale.bandwidth())
    .attr('height', (d) => height - yScale(d[yAxisColumn]))
//    .attr('fill', 'rgba(75, 192, 192, 0.7)')
    .attr('fill', (d, i) => colorScale(i)) // U
...
export default FootballBarChart;
